...
class chatBackendController extends BackendResourceController
{
    ...
    function action_getActiveRoom()
    {
        if (isset($_SESSION['frontend_active_room_id'])) {
            $this->sendResponse(200, array(
                "room_id" => $_SESSION['frontend_active_room_id'],
                "member_id" => $_SESSION['frontend_active_member_id']
            ));
        } else {
            ...
            if ($room) {
                ...
                if (isset($_SESSION['joinedroom_member_id'][$roomId])) {
                    ...
                    if ($qry->num_rows < $room['max_member']) {
                        ...
                        $message = array(
                            "event" => "connected",
                            "message" => $currentUser->user_name . " joined."
                        );
    ...
    function action_ajaxDisconnectChat()
    {
        if (isset($_REQUEST['room_id']) && $_REQUEST['member_id']) {
            ...
            $this->broadcastMessageToRoom($roomId, $memberId, array(
                "event" => "disconnected",
                "message" => $member['name'] . " left the chat."
            ));
