...
class rolesViewDetail  extends ViewDetail {
    ...
    function processCheckBoxes($row) {
        ...
        if($row['module_access']=="yes") {
            $row['module_access'] = '<input type="checkbox" name="module_access[]" value="'.$row['module_id'].'" checked="checked" />';
        } else {
            $row['module_access'] = '<input type="checkbox" name="module_access[]"  value="'.$row['module_id'].'"  />';
        }
    ...
    function afterDisplay() {
        ...
        $rows = $db->fetchRows($sql,array("module_id"));
        $headers = array();
